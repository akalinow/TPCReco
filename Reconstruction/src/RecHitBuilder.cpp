...
TH2D RecHitBuilder::makeCleanCluster(const TH2D & aHisto){
  ...
  kernelSumThreshold = 300;//parameter to moved to configuration
...
std::vector<TH2D*> RecHitBuilder::Get2D(const SigClusterTPC &cluster, double radius, int rebin_space, int rebin_time, int method) { 
  ...
  if(!IsOK() || !cluster.IsOK() || 
     cluster.GetNhits(definitions::projection_type::DIR_U)<1 || cluster.GetNhits(definitions::projection_type::DIR_V)<1 || cluster.GetNhits(definitions::projection_type::DIR_W)<1 ) return hvec;

  // loop over time slices and match hits in space
  const int time_cell_min = MAXIMUM( cluster.min_time[definitions::projection_type::DIR_U], MAXIMUM( cluster.min_time[definitions::projection_type::DIR_V], cluster.min_time[definitions::projection_type::DIR_W] ));
  const int time_cell_max = MINIMUM( cluster.max_time[definitions::projection_type::DIR_U], MINIMUM( cluster.max_time[definitions::projection_type::DIR_V], cluster.max_time[definitions::projection_type::DIR_W] ));
  ...
  for(int icell=time_cell_min; icell<=time_cell_max; icell++) {
    if((hitListByTimeDirMerged.find(MultiKey2(icell, definitions::projection_type::DIR_U))==hitListByTimeDirMerged.end()) ||
       (hitListByTimeDirMerged.find(MultiKey2(icell, definitions::projection_type::DIR_V))==hitListByTimeDirMerged.end()) ||
       (hitListByTimeDirMerged.find(MultiKey2(icell, definitions::projection_type::DIR_W))==hitListByTimeDirMerged.end())) continue;
    
    std::vector<int> hits[3] = {
				hitListByTimeDirMerged.find(MultiKey2(icell, definitions::projection_type::DIR_U))->second,
				hitListByTimeDirMerged.find(MultiKey2(icell, definitions::projection_type::DIR_V))->second,
				hitListByTimeDirMerged.find(MultiKey2(icell, definitions::projection_type::DIR_W))->second};

    ////////// DEBUG 
    //   std::cout << Form(">>>> Number of hits: time cell=%d: U=%d / V=%d / W=%d",
    //		      icell, (int)hits[definitions::projection_type::DIR_U].size(), (int)hits[definitions::projection_type::DIR_V].size(), (int)hits[definitions::projection_type::DIR_W].size()) << std::endl;
    ////////// DEBUG 

    // check if there is at least one hit in each direction
    if(hits[definitions::projection_type::DIR_U].size()==0 || hits[definitions::projection_type::DIR_V].size()==0 || hits[definitions::projection_type::DIR_W].size()==0) continue;
    ...
    for(int i0=0; i0<(int)hits[0].size(); i0++) {
      for(auto iter0=myGeometryPtr->GetDirSectionIndexList(definitions::projection_type::DIR_U).begin();
	  iter0!=myGeometryPtr->GetDirSectionIndexList(definitions::projection_type::DIR_U).end(); iter0++) {
	std::shared_ptr<StripTPC> strip0 = myGeometryPtr->GetStripByDir(definitions::projection_type::DIR_U, *iter0, hits[0].at(i0));
	for(int i1=0; i1<(int)hits[1].size(); i1++) {
	  for(auto iter1=myGeometryPtr->GetDirSectionIndexList(definitions::projection_type::DIR_V).begin();
	      iter1!=myGeometryPtr->GetDirSectionIndexList(definitions::projection_type::DIR_V).end(); iter1++) {
	    std::shared_ptr<StripTPC> strip1 = myGeometryPtr->GetStripByDir(definitions::projection_type::DIR_V, *iter1, hits[1].at(i1));
	    for(int i2=0; i2<(int)hits[2].size(); i2++) {
	      for(auto iter2=myGeometryPtr->GetDirSectionIndexList(definitions::projection_type::DIR_W).begin();
		  iter2!=myGeometryPtr->GetDirSectionIndexList(definitions::projection_type::DIR_W).end(); iter2++) {
		std::shared_ptr<StripTPC> strip2 = myGeometryPtr->GetStripByDir(definitions::projection_type::DIR_W, *iter2, hits[2].at(i2));
		...
		if( myGeometryPtr->MatchCrossPoint( strip0, strip1, strip2, radius, pos )) {
		  (n_match[definitions::projection_type::DIR_U])[hits[0].at(i0)]++;
		  (n_match[definitions::projection_type::DIR_V])[hits[1].at(i1)]++;
		  (n_match[definitions::projection_type::DIR_W])[hits[2].at(i2)]++;
    ...
    for(it1=hitPos.begin(); it1!=hitPos.end(); it1++) {
      ...
      q[definitions::projection_type::DIR_U] = GetValByStripMerged(definitions::projection_type::DIR_U, u1, icell);
      q[definitions::projection_type::DIR_V] = GetValByStripMerged(definitions::projection_type::DIR_V, v1, icell);
      q[definitions::projection_type::DIR_W] = GetValByStripMerged(definitions::projection_type::DIR_W, w1, icell);
	...
	if(u1==u2) {
	  qtot[definitions::projection_type::DIR_V] += GetValByStripMerged(definitions::projection_type::DIR_V, v2, icell);
	  qtot[definitions::projection_type::DIR_W] += GetValByStripMerged(definitions::projection_type::DIR_W, w2, icell);
	}
	if(v1==v2) {
	  qtot[definitions::projection_type::DIR_W] += GetValByStripMerged(definitions::projection_type::DIR_W, w2, icell);
	  qtot[definitions::projection_type::DIR_U] += GetValByStripMerged(definitions::projection_type::DIR_U, u2, icell);
	}
	if(w1==w2){
	  qtot[definitions::projection_type::DIR_U] += GetValByStripMerged(definitions::projection_type::DIR_U, u2, icell);
	  qtot[definitions::projection_type::DIR_V] += GetValByStripMerged(definitions::projection_type::DIR_V, v2, icell);
      ...
      fraction[definitions::projection_type::DIR_U].insert(std::pair<MultiKey3, double>(it1->first, q[definitions::projection_type::DIR_U] / qtot[definitions::projection_type::DIR_U] ));
      fraction[definitions::projection_type::DIR_V].insert(std::pair<MultiKey3, double>(it1->first, q[definitions::projection_type::DIR_V] / qtot[definitions::projection_type::DIR_V] ));
      fraction[definitions::projection_type::DIR_W].insert(std::pair<MultiKey3, double>(it1->first, q[definitions::projection_type::DIR_W] / qtot[definitions::projection_type::DIR_W] ));
    }
	...
	case 0: // mehtod #1 - divide charge equally 
	  val = 
	    GetValByStripMerged(definitions::projection_type::DIR_U, std::get<0>(it->first), icell) / n_match[0].at(std::get<0>(it->first)) +
	    GetValByStripMerged(definitions::projection_type::DIR_V, std::get<1>(it->first), icell) / n_match[1].at(std::get<1>(it->first)) +
	    GetValByStripMerged(definitions::projection_type::DIR_W, std::get<2>(it->first), icell) / n_match[2].at(std::get<2>(it->first));
	  break;

	case 1: // method #2 - divide charge according to charge-fraction in two other directions

	  val = 
	    GetValByStripMerged(definitions::projection_type::DIR_U, 
				std::get<0>(it->first), icell)*0.5*( fraction[definitions::projection_type::DIR_V].at(it->first) + fraction[definitions::projection_type::DIR_W].at(it->first) ) +
	    GetValByStripMerged(definitions::projection_type::DIR_V, 
				std::get<1>(it->first), icell)*0.5*( fraction[definitions::projection_type::DIR_W].at(it->first) + fraction[definitions::projection_type::DIR_U].at(it->first) ) +
	    GetValByStripMerged(definitions::projection_type::DIR_W, 
				std::get<2>(it->first), icell)*0.5*( fraction[definitions::projection_type::DIR_U].at(it->first) + fraction[definitions::projection_type::DIR_V].at(it->first) );
	  break;
...
TH3D *RecHitBuilder::Get3D(const SigClusterTPC &cluster, double radius, int rebin_space, int rebin_time, int method) {
  ...
  if(!IsOK() || !cluster.IsOK() || 
     cluster.GetNhits(definitions::projection_type::DIR_U)<1 || cluster.GetNhits(definitions::projection_type::DIR_V)<1 || cluster.GetNhits(definitions::projection_type::DIR_W)<1 ) return h;
  ...
  for(int icell=time_cell_min; icell<=time_cell_max; icell++) {
    ...
    std::vector<int> hits[3] = {
      ...
      //if((hitListByTimeDir.find(MultiKey2(icell, definitions::projection_type::DIR_U))==hitListByTimeDir.end()) ||
      //(hitListByTimeDir.find(MultiKey2(icell, definitions::projection_type::DIR_V))==hitListByTimeDir.end()) ||
      //(hitListByTimeDir.find(MultiKey2(icell, definitions::projection_type::DIR_W))==hitListByTimeDir.end())) continue;
      
      //std::vector<int> hits[3] = {
      //hitListByTimeDir.find(MultiKey2(icell, definitions::projection_type::DIR_U))->second,
      //hitListByTimeDir.find(MultiKey2(icell, definitions::projection_type::DIR_V))->second,
      //hitListByTimeDir.find(MultiKey2(icell, definitions::projection_type::DIR_W))->second};
