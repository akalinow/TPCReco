...
StripResponseCalculator::StripResponseCalculator(std::shared_ptr<GeometryTPC> aGeometryPtr,
                                                 ...
                                                 double sigma_xy,
                                                 double sigma_z, // horizontal and vertical gaussian spread [mm]
        ...
        Npads(abs(delta_pads)),
        sigma_xy(sigma_xy), sigma_z(sigma_z), peaking_time(peaking_time), has_UVWprojectionsRaw(false),
    ...
    if (debug_flag) {
        for (auto &it: responseMapPerMergedStrip) {
            std::cout << __FUNCTION__ << ": " << it.second->GetName() << ", integral=" << it.second->Integral()
                      << std::endl;
        }
...
bool StripResponseCalculator::loadHistograms(const char *fname) {
    ...
    if (!f.IsOpen()) {
        if (debug_flag)
            std::cout << __FUNCTION__ << KRED << ": Cannot open TFile: " << fname << "!" << RST << std::endl;
        return false;
    }
    ...
    for (int istrip = -Nstrips; istrip <= Nstrips; istrip++) {
        for (int strip_dir = definitions::projection_type::DIR_U;
             strip_dir <= definitions::projection_type::DIR_W; strip_dir++) {
            auto hist = (TH2D *) f.Get(getStripResponseHistogramName(strip_dir, istrip));
            if (!hist) {
                if (debug_flag)
                    std::cout << __FUNCTION__ << KRED << ": Cannot find histogram: "
                              << getStripResponseHistogramName(strip_dir, istrip) << "!" << RST << std::endl;
                return false;
            }
    ...
    for (int istrip = -Nstrips; istrip <= Nstrips; istrip++) {
        for (int strip_dir = definitions::projection_type::DIR_U;
            ...
            for (int ipad = -Npads;
                 ipad <= Npads + abs((istrip % 2)); ipad++) { // add 1 extra pad for odd relative strip index
                auto hist = (TH2D *) f.Get(getStripSectionStartResponseHistogramName(strip_dir, istrip, ipad));
                if (!hist) {
                    if (debug_flag)
                        std::cout << __FUNCTION__ << KRED << ": Cannot find histogram: "
                                  << getStripSectionStartResponseHistogramName(strip_dir, istrip, ipad) << "!" << RST
                                  << std::endl;
                    return false;
                }
                hist->SetDirectory(0); // do not associate this histogram with any TFile directory
                ...
                ////// DEBUG
                if (debug_flag)
                    std::cout << __FUNCTION__ << ": dir=" << strip_dir << ", delta_strip=" << istrip << ", delta_pad="
                              << ipad << ", hist=" << hist << std::endl;
                ////// DEBUG
            }
        }
    }
    ...
    for (auto icell = -Ntimecells; icell <= Ntimecells; icell++) {
        ...
        if (!hist) {
            if (debug_flag)
                std::cout << __FUNCTION__ << KRED << ": Cannot find histogram: " << getTimeResponseHistogramName(icell)
                          << "!" << RST << std::endl;
            return false;
        }
        hist->SetDirectory(0); // do not associate this histogram with any TFile directory
...
bool StripResponseCalculator::saveHistograms(const char *fname) {
    ...
    if (!f.IsOpen()) {
        if (debug_flag)
            std::cout << __FUNCTION__ << KRED << ": Cannot create new TFile: " << fname << "!" << RST << std::endl;
        return false;
    }
    f.cd();
    for (auto &it: responseMapPerMergedStrip) {
...
bool StripResponseCalculator::setUVWprojectionsRaw(std::vector<TH2D *> aUVWprojectionsRaw) {
    ...
    if (aUVWprojectionsRaw.size() != 3) {
        if (debug_flag)
            std::cout << __FUNCTION__ << KRED << " Invalid size of TH2D histogram array!" << RST << std::endl;
        return false;
    }
    for (auto &it: aUVWprojectionsRaw) {
        if (!it) {
            if (debug_flag)
                std::cout << __FUNCTION__ << KRED << " Invalid pointer to TH2D histogram!" << RST << std::endl;
            return false;
        }
    }
...
bool StripResponseCalculator::setUVWprojectionsRaw(
        std::vector<std::shared_ptr<TH2D> > aUVWprojectionsRaw) { // pair={STRIP_DIR, TH2D pointer}
    std::vector<TH2D *> vec;
...
bool StripResponseCalculator::setUVWprojectionsInMM(
        std::vector<std::shared_ptr<TH2D> > aUVWprojectionsInMM) { // pair={STRIP_DIR, TH2D pointer}
    std::vector<TH2D *> vec;
...
void
StripResponseCalculator::addCharge(double x, double y, double z, double charge, std::shared_ptr<PEventTPC> aEventPtr) {
    ...
    const auto refStrips = getReferenceStripNode(x, y,
                                                 &refNodePosInMM); // vector with {u0, v0, w0} triplet, range [1-1024]
    ...
    ////// DEBUG
    if (debug_flag)
        std::cout << __FUNCTION__ << ": Relative postion=[dx=" << dx << ", dy=" << dy << ", dz=" << dz << "]"
                  << ", Charge=" << charge
    ...
    for (auto &respXY: responseMapPerMergedStrip) {
        ...
        if (debug_flag) {
            ...
            for (auto &it: list) {
                std::cout << "    point=[" << it.pos.X() << ", " << it.pos.Y() << "], prev_sec=" << it.previous
                          << ", next_sec=" << it.next << std::endl;
            }
        }
        ////// DEBUG
        ...
        for (auto &it: list) {
            if (it.next != GeometryTPC::outside_section &&
                fractionPerSectionMap.find(it.next) == fractionPerSectionMap.end())
                fractionPerSectionMap[it.next] = smeared_fractionXY;
            if (it.previous != GeometryTPC::outside_section &&
                fractionPerSectionMap.find(it.previous) == fractionPerSectionMap.end())
        ...
        for (auto &it: list) {
            const auto delta_pads = (int) TMath::Ceil(
                    ((it.pos - refNodePosInMM) * myGeometryPtr->GetStripUnitVector(smeared_strip_dir)) /
                    myGeometryPtr->GetPadPitch()
            ...
            if (delta_pads < -Npads || delta_pads > Npads + abs(smeared_strip_num - refStrips[smeared_strip_dir]) % 2) {
                ...
                if (delta_pads < -Npads && it.previous != GeometryTPC::outside_section) {
                    ...
                    if (debug_flag)
                        std::cout << __FUNCTION__ << ": Unmerged strip "
                                  << myGeometryPtr->GetDirName(smeared_strip_dir) << smeared_strip_num
                                  << " [dir=" << smeared_strip_dir << ", sec=" << it.previous << ", num="
                                  << smeared_strip_num << "]"
                                  << ": delta_pads=" << delta_pads
                                  << " => zeroing sec=" << it.previous << ": new_val=0 (old="
                                  << fractionPerSectionMap[it.previous] << ")" << std::endl;
                    ////// DEBUG

                    fractionPerSectionMap[it.previous] = 0.0; // no impact on section it.next
                }
                if (delta_pads > Npads + abs(smeared_strip_num - refStrips[smeared_strip_dir]) % 2 &&
                    it.next != GeometryTPC::outside_section) {

                    ////// DEBUG
                    if (debug_flag)
                        std::cout << __FUNCTION__ << ": Unmerged strip "
                                  << myGeometryPtr->GetDirName(smeared_strip_dir) << smeared_strip_num
                                  << " [dir=" << smeared_strip_dir << ", sec=" << it.next << ", num="
                                  << smeared_strip_num << "]"
                                  << ": delta_pads=" << delta_pads
                                  << " => zeroing sec=" << it.next << ": new_val=0 (old="
                                  << fractionPerSectionMap[it.next] << ")" << std::endl;
                    ////// DEBUG
            ...
            const auto it2 = responseMapPerStripSectionStart.find(
            ...
            if (it.previous != GeometryTPC::outside_section) {
                ...
                if (debug_flag)
                    std::cout << __FUNCTION__ << ": Unmerged strip "
                              ...
                              << smeared_strip_num << "]"
                              << ": delta_pads=" << delta_pads
                              << " => subtracting from sec=" << it.previous << ": new_val="
                              << fractionPerSectionMap[it.previous]
                              << std::endl;
                ////// DEBUG
            }
            if (it.next != GeometryTPC::outside_section) {
                ...
                if (debug_flag)
                    std::cout << __FUNCTION__ << ": Unmerged strip "
                              ...
                              << "]"
                              << ": delta_pads=" << delta_pads
                              << " => subtracting from sec=" << it.next << ": new_val="
                              << fractionPerSectionMap[it.next]
                              << std::endl;
                ////// DEBUG
            }
        }
        ...
        for (auto &it2: fractionPerSectionMap) {
            ...
            if (debug_flag)
                std::cout << __FUNCTION__ << ": Unmerged strip "
                          << myGeometryPtr->GetStripName(
                                  myGeometryPtr->GetStripByDir(smeared_strip_dir, it2.first, smeared_strip_num))
                          << " [dir=" << smeared_strip_dir << ", sec=" << it2.first << ", num=" << smeared_strip_num
                          << "]"
                          << ": total_charge_fraction=" << smeared_fractionXY_per_section
                          << ", merged_strip_fraction=" << smeared_fractionXY_per_section / smeared_fractionXY
                          << std::endl;
            ////// DEBUG
        }
        ////// DEBUG
        ...
        for (auto &respZ: responseMapPerTimecell) {
            ...
            if (has_UVWprojectionsRaw) {
                fillUVWprojectionsRaw[smeared_strip_dir]->Fill(smeared_timecell * 1., smeared_strip_num * 1.,
                                                               smeared_charge);
                ////// DEBUG
            ...
            if (aEventPtr) {
                for (auto &it2: fractionPerSectionMap) {
                    ...
                    std::shared_ptr<StripTPC> aStrip = myGeometryPtr->GetStripByDir(smeared_strip_dir, it2.first,
                                                                                    smeared_strip_num);
...
std::vector<int> StripResponseCalculator::getReferenceStripNode(double x, double y, TVector2 *refNodePosInMM) const {
    ...
    ////// DEBUG
    if (debug_flag)
        std::cout << __FUNCTION__ << ": strip_dir=" << strip_dir << ", strip_num=" << stripMap[strip_dir] << std::endl;
    ////// DEBUG

    const auto strip_startPos = strip->Start(); // absolute position of strip's starting point
    const auto distance_to_start =
            (TVector2(x, y) - strip_startPos) * myGeometryPtr->GetStripUnitVector(strip_dir); // position wrt start
    auto distance_to_node = fmod(distance_to_start, myGeometryPtr->GetPadPitch()); // position wrt start
    if (distance_to_node > 0.5 * myGeometryPtr->GetPadPitch())
        distance_to_node -= myGeometryPtr->GetPadPitch(); // range [-0.5*pad_pitch, 0.5*pad_pitch]
    const auto nodePos = strip_startPos + myGeometryPtr->GetStripUnitVector(strip_dir) * (distance_to_start -
                                                                                          distance_to_node); // absolute position of the nearest strip node

    // find strip numbers for 2 complementary strip directions
    for (int check_dir = definitions::projection_type::DIR_U;
         check_dir <= definitions::projection_type::DIR_W; check_dir++) {
        if (check_dir == strip_dir) continue;

        for (auto isign = -1; isign <= 1; isign += 2) { // probe 2 adjacent pads for each direction index
            const auto checkPos =
                    nodePos + myGeometryPtr->GetStripUnitVector(check_dir) * 0.5 * myGeometryPtr->GetPadPitch() * isign;
            const auto check_strip = myGeometryPtr->GetTH2PolyStrip(
                    myGeometryPtr->GetTH2Poly()->FindBin(checkPos.X(), checkPos.Y()));
            if (!check_strip) continue;
            stripMap[check_dir] = check_strip->Num();

            if (debug_flag)
                std::cout << __FUNCTION__ << ": strip_dir=" << check_dir << ", strip_num=" << stripMap[check_dir]
                          << std::endl;
    ...
    if (stripMap.size() != 3) {
        if (debug_flag)
            std::cout << __FUNCTION__
                      << KRED << ": Found only " << stripMap.size()
                      << " matching strips out of 3 for given XY position!"
                      << RST << std::endl;
        return result; // return empty vector on error
    }
...
void StripResponseCalculator::initializeStripResponse(unsigned long NpointsXY, int NbinsXY) {
    ...
    ////// DEBUG
    if (debug_flag) {
        std::cout << __FUNCTION__ << ": responseMapPerMergedStrip size=" << responseMapPerMergedStrip.size()
                  << std::endl;
        std::cout << __FUNCTION__ << ": responseMapPerSectionStripStart size=" << responseMapPerStripSectionStart.size()
                  << std::endl;
    }
    ////// DEBUG
    ...
    const auto refStrips = getReferenceStripNode(0.5 * (xmin + xmax), 0.5 * (ymin + ymax),
                                                 &refNodePosInMM); // vector with {u0, v0, w0} triplet, range [1-1024]
    ...
    TF2 gauss2d("f_gauss2d", [](double *x, double *p) {
                    return 1. / TMath::TwoPi() / p[0] / p[0] * exp(-0.5 * (x[0] * x[0] + x[1] * x[1]) / p[0] / p[0]);
                },
    ...
    const auto R2 = pow(myGeometryPtr->GetPadSize() +
                        sqrt(pow(hist->GetXaxis()->GetBinWidth(1), 2) + pow(hist->GetYaxis()->GetBinWidth(1), 2)),
                        2); // [mm^2]
    for (unsigned long ipoint = 0L; ipoint < NpointsXY; ipoint++) {
        ...
        for (auto ibin1 = 1; ibin1 <= hist->GetNbinsX(); ibin1++) {
            ...
            for (auto ibin2 = 1; ibin2 <= hist->GetNbinsY(); ibin2++) {
                ...
                const auto strip = myGeometryPtr->GetTH2PolyStrip(
                        myGeometryPtr->GetTH2Poly()->FindBin(refNodePosInMM.X() + x, refNodePosInMM.Y() + y));
                if (!strip) continue;

                // fill charge fraction for merged strips
                const auto it = responseMapPerMergedStrip.find(
                        MultiKey2(strip->Dir(), strip->Num() - refStrips[strip->Dir()]));
                ...
                */
                const auto delta_pads = (int) TMath::Ceil(
                        (TVector2(x, y) * myGeometryPtr->GetStripUnitVector(strip->Dir())) /
                        myGeometryPtr->GetPadPitch()
                        + ((strip->Num() - refStrips[strip->Dir()]) % 2 == 0 ? 0.0 : 0.5));
                for (auto ipad = std::max(-Npads, delta_pads);
                     ipad <= Npads + abs(strip->Num() - refStrips[strip->Dir()]) % 2; ipad++) {
                    const auto it2 = responseMapPerStripSectionStart.find(
                            MultiKey3(strip->Dir(), strip->Num() - refStrips[strip->Dir()], ipad));
    ...
    if (debug_flag) {
        ...
        for (auto ibin1 = 1; ibin1 <= hist->GetNbinsX(); ibin1++) {
            ...
            for (auto ibin2 = 1; ibin2 <= hist->GetNbinsY(); ibin2++) {
                ...
                if (sum == 0) continue;
                std::cout << "center=[" << c1 << ", " << c2 << "], total_integral=" << sum << " (expected 1)"
                          << std::endl;
                ...
                for (auto &it3: responseMapPerStripSectionStart) {
                    ...
                    std::cout << it3.second->GetName()
                              << ": center=[" << c1 << ", " << c2 << "], merged_strip_fraction="
                              << frac_per_section / frac_per_strip << std::endl;
                }
            }
        }
        std::cout << __FUNCTION__ << ": Initialized " << responseMapPerMergedStrip.size()
                  << " horizontal response 2D histograms (merged strips)" << std::endl;
        std::cout << __FUNCTION__ << ": Initialized " << responseMapPerStripSectionStart.size()
                  << " horizontal response 2D histograms (section start position)." << std::endl;
    }
    ////// DEBUG
}
...
void StripResponseCalculator::initializeTimeResponse(unsigned long NpointsPeakingTime, int NbinsZ) {
    ...
    if (debug_flag) {
        ...
        std::cout << __FUNCTION__ << ": Initialized " << responseMapPerTimecell.size()
                  << " vertical response 1D histograms (GAUSS only)." << std::endl;
    }
    ////// DEBUG
    ...
    if (peaking_time > 0) {
        ...
        TF1 responseShape1d("f_responseShape1d", [](double *x, double *p) {
                            ...
                            0.0, myGeometryPtr->GetTimeBinWidth() / factor_ns2mm + peaking_time * 4, 3,
                            1); // 3 parameters, 1 dimension
        ...
        for (unsigned long ipoint = 0L; ipoint < NpointsPeakingTime; ipoint++) {
            ...
            for (auto &it: responseMapPerTimecell) {
                ...
                for (auto ibin = 1; ibin <= hist->GetNbinsX(); ibin++) {
                    ...
                    const auto weight =
                            charge_frac / (double) NpointsPeakingTime; // contribution of a single sampling point
                    ...
                    if (responseMapPerTimecellWithPeakingTime.find(index_fill) !=
                        responseMapPerTimecellWithPeakingTime.end()) {
    ...
    if (debug_flag) {
        ...
        std::cout << __FUNCTION__ << ": Initialized " << responseMapPerTimecell.size()
                  << " vertical response 1D histograms (GAUSS + PEAKING TIME)." << std::endl;
    }
    ////// DEBUG
}
...
std::shared_ptr<TH2D>
StripResponseCalculator::getStripSectionStartResponseHistogram(int dir, int delta_strip, int delta_pad) {
...
const char *
StripResponseCalculator::getStripSectionStartResponseHistogramName(int dir, int delta_strip, int delta_pad) {
...
const char *StripResponseCalculator::getTimeResponseHistogramName(int delta_timecell) {
    return Form("h_respZ_%s%d", (delta_timecell == 0 ? "" : (delta_timecell < 0 ? "minus" : "plus")),
                abs(delta_timecell));
}

std::string
StripResponseCalculator::generateRootFileName(int nStrips, int nCells, int nPads, double sigmaXY, double sigmaZ,
                                              double peakingTime, double samplingRate, double vDrift) {
    std::string result;
    if (peakingTime == 0) {
        result = Form("StripResponseModel_%dx%dx%d_S%gMHz_V%gcmus_T%gmm_L%gmm.root", nStrips, nCells, nPads,
                      samplingRate, vDrift, sigmaXY, sigmaZ);
    }
    else{
        result = Form("StripResponseModel_%dx%dx%d_S%gMHz_V%gcmus_T%gmm_L%gmm_P%gns.root", nStrips, nCells, nPads,
                      samplingRate, vDrift, sigmaXY, sigmaZ, peakingTime);
    }
    return result;
}
