...
public:
    ...
    bool
    setUVWprojectionsInMM(std::vector<std::shared_ptr<TH2D> > aUVWprojectionsInMM); // pair={STRIP_DIR, TH2D pointer}
    ...
    void addCharge(TVector3 position3d, double charge,
                   std::shared_ptr<PEventTPC> aEventPtr = std::shared_ptr<PEventTPC>(nullptr));
    ...
    static std::string
    generateRootFileName(int nStrips, int nCells, int nPads, double sigmaXY, double sigmaZ, double peakingTime,
                         double samplingRate, double vDrift);
...
private:
    ...
    int Ntimecells{0};
    int Npads{0};
    static const int default_NbinsZ{
            20};  // {5}; // granularity of strip response histograms in Z domain (sub-time cell resolution)
    static const int default_NbinsXY{
            10}; // {7}; // granularity of strip response histograms in X/Y domain (sub-strip resolution)
    static const unsigned long default_NpointsXY{
            10000}; // # of sampling points for initialization of strip response histograms
    //// TEST
