...
void MainFrame::Update() {
	...
	//myHistoManager.getCartesianProjection(U)->SaveAs("histo.root");
	...
	//no checking for nullptr below, because bad allocation throws exception anyways
	...
	aPad = fCanvas->cd(8);
	auto h2D = myHistoManager.get2DReconstruction();
	h2D->Draw("colz");
...
void MainFrame::HandleMenu(int32_t id) {
	...
	case M_NEXT_EVENT:
	{
		myDataManager.loadNextEvent();
		Update();
	}
	break;
	case M_PREVIOUS_EVENT:
	{
		myDataManager.loadPreviousEvent();
		Update();
	}
	break;
	case M_GOTO_EVENT:
