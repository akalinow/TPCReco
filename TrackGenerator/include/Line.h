...
public:
	...
	Line(TRandom3* rng, TVector3 stop = { 0, 0, 1 }, TVector3 start = { 0, 0, 0 }) :
		rng(rng), start(start), stop(stop), track(stop - start),
		randomLength(false), randomTheta(false), randomPhi(false)
	{}
	Line(TRandom3* rng, double length, double theta, double phi, TVector3 start = { 0, 0, 0 }) :
	...
	void update() {
		...
		if (randomTheta) {
			...
			setTheta(std::acos(cos));
		}
		if (randomPhi) {
			setPhi(rng->Uniform(2 * pi));
		}
	}
	...
	inline void setStart(double x, double y, double z) { setStart({ x, y, z }); } //mm
	inline void setStop(const TVector3& s) { stop = s; track = stop - start; } //mm
	//  inline void setStop(TVector3&& s){stop=s;track=stop-start;}
	inline void setStop(double x, double y, double z) { setStop({ x, y, z }); } //mm
