...
G4VSolid *VolumeBuilder::BuildSolid(const SolidDescriptor &solid) {
    auto mesh = new CADMesh((char *) solid.filename.c_str());
    G4VSolid *cad_solid = mesh->TessellatedMesh();
    solids[solid.name] = cad_solid;
    return cad_solid;
}

G4LogicalVolume *VolumeBuilder::BuildLogical(const SolidDescriptor &solid) {
    auto log_vol = new G4LogicalVolume(BuildSolid(solid), material_builder->GetMaterial(solid.material),
                                       G4String(solid.name + "_logical"));
    auto attrs = new G4VisAttributes(solid.color);
...
void VolumeBuilder::BuildVolumes(G4LogicalVolume *mother_log) {
    ...
    for (const auto &solid: geo_config->GetSolids()) {
