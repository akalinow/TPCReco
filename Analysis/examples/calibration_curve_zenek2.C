...
//
// root
// root [0] .L calibration_curve_zenek2.C
// root [1] calibration_curve_zenek2()
//
//
//////////////////////////
//////////////////////////
...
void calibration_curve_zenek2(double scale=1.03499, double offset_mm=1.6402,
  ...
  auto gr_data_Eexpected_vs_Euncorr=new TGraph();
  ...
  if(beforeFitName!=NULL && afterFitName!=NULL) {
	...
	auto Ex_expected = l->GetX1(); // MeV, expected corrected Ex peak position
	...
	auto T_alpha_CMS_expected = (Ex_expected+Qvalue) / (1.0 + mass_alpha/mass_C12); // MeV
	//	auto T_alpha_CMS_corr = (Ex_corr+Qvalue) / (1.0 + mass_alpha/mass_C12); // MeV
	
	// nominal boost
	auto Egamma_CMS_expected=  Ex_expected*(1.0 - 0.5*Ex_expected/(mass_O16+Ex_expected)); // MeV
	auto Egamma_LAB_expected = 0.5*( pow(mass_O16 + Ex_expected, 2) / mass_O16 - mass_O16 ); // MeV
	auto beta_CMS_wrt_LAB = Egamma_LAB_expected / (Egamma_LAB_expected+mass_O16)*TVector3(0,0,1); // along Z-axis in BEAM coordinate system
	...
	auto p_alpha_CMS_expected = sqrt(T_alpha_CMS_expected*(T_alpha_CMS_expected+2*mass_alpha)); // expected momentum in CMS
	TLorentzVector P4_alpha_CMS_expected; // BEAM coordinates
	P4_alpha_CMS_expected.SetVectM(p_alpha_CMS_expected*TVector3(0,0,1), mass_alpha); // expected 4-momentum in CMS corresponding to theta_BEAM=0, BEAM coordinates
	...
	auto P4_alpha_LAB_expected(P4_alpha_CMS_expected); // expected 4-momentum in LAB corresponding to theta_BEAM=0, BEAM coordinates
	P4_alpha_LAB_expected.Boost(1.0*beta_CMS_wrt_LAB); // CMS to LAB, see ROOT documentation for sign convention
	...
	auto T_alpha_LAB_expected = P4_alpha_LAB_expected.E()-mass_alpha; // MeV, expected kinetic energy in LAB
	//	auto T_alpha_LAB_corr= P4_alpha_LAB_corr.E()-mass_alpha; // MeV, corrected kinetic energy in LAB
	
	std::cout << "Fitted  Ex_uncorr[MeV]=" << Ex_uncorr << "  Ex_expected[MeV]=" << Ex_expected
		  << " :  E_LAB_uncorr(theta=0)[MeV]=" << T_alpha_LAB_uncorr
		  << "  E_LAB_expected(theta=0)[MeV]=" << T_alpha_LAB_expected
		  << "  (diff=" << (T_alpha_LAB_expected-T_alpha_LAB_uncorr)/T_alpha_LAB_uncorr*100 << " \%)" << std::endl;
	...
	gr_data_Eexpected_vs_Euncorr->SetPoint(gr_data_Eexpected_vs_Euncorr->GetN(), T_alpha_LAB_uncorr, T_alpha_LAB_expected);
      }
    }
  }
  ///////////
  ...
  */
  ...
  c->Clear();
  gr_Ecorr_vs_Euncorr->Draw("AP");
  gr_Ecorr_vs_Euncorr->SetMinimum(1.25);
  gr_Ecorr_vs_Euncorr->SetMaximum(4.95);
  gr_Ecorr_vs_Euncorr->GetXaxis()->SetRangeUser( 1.25, 4.95);
  //  gr_Ecorr_vs_Euncorr->GetXaxis()->SetRangeUser( Emin, Emax );
  //  gr_Ecorr_vs_Euncorr->GetYaxis()->SetRangeUser( Emin, Emax );
  gr_Ecorr_vs_Euncorr->Draw("L SAME");
  gr_Ecorr_vs_Euncorr->GetXaxis()->SetTitle("Raw #alpha-particle E_{KIN} in LAB  [MeV]");
  ...
  if(gr_data_Eexpected_vs_Euncorr->GetN()>0) {
    gr_data_Eexpected_vs_Euncorr->SetMarkerColor(kBlack);
    gr_data_Eexpected_vs_Euncorr->SetMarkerSize(1.5);
    //    gr_data_Eexpected_vs_Euncorr->SetMarkerStyle(kCircle);
    //    gr_data_Eexpected_vs_Euncorr->SetMarkerStyle(kFullDotLarge);
    gr_data_Eexpected_vs_Euncorr->SetMarkerStyle(kFullSquare);
    gr_data_Eexpected_vs_Euncorr->Draw("P SAME");
  }

  auto gr_null=new TGraph(2);
  gr_null->SetPoint(0, Emin, Emin);
  gr_null->SetPoint(1, Emax, Emax);
  gr_null->Draw("L SAME");
  gr_null->SetLineColor(kBlue);
  gr_null->SetLineStyle(kDashed);
  gr_null->SetLineWidth(2);
    
  c->Modified();
  c->Update();
  c->Print("calibration_curve3.root");
  c->Print("calibration_curve3.png");
