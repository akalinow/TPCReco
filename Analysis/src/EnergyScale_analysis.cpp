...
double EnergyScale_analysis::getOptimalBinWidthInMeV(double histogramRMS, double histogramIntegral) const {
  ...
  auto binMin = std::max( resolution, histogramRMS/10); // at least 20 bins  in [-sigma, sigma] range
  auto binOpt = 2 * histogramRMS / log(1+histogramIntegral); // 4 * histogramRMS / log(1+histogramIntegral);
  ...
  if (bin<0.02) return 2 * resolution * (long)( bin / resolution / 2); // rounded off to 2 keV
...
std::tuple<double, size_t> EnergyScale_analysis::getSelectionChi2(EventCollection &selection, bool debug_histos_flag) {
  ...
  if(result1.Get()) {
    auto factor = pow( Qvalue_mean_err, 2) + pow( Qvalue_fit_err, 2) + // + pow( Qvalue_fit_err, 2) +
      ( USE_EXPECTED_ERROR ? pow(selection.expectedExcitationEnergySigmaInMeV_CMS, 2) : 0.0 ) +
      pow(myOptions.detectorQvalueResolutionInMeV_CMS, 2);
...
std::tuple<double, double, bool> EnergyScale_analysis::getEventExcitationEnergy(double nominalBeamEnergyInMeV_LAB,
  ...
  //  auto average_Qvalue_CMS=parentMassGroundState+expectedExcitationEnergyPeakInMeV_CMS-leading_mass-trailing_mass;
...
///////////////////////////////////////////////////////// - IMPLEMENTATION #8
//
// helper function to calculate optimal bin size [MeV] for excitattion ennergy histogram
// rounded off to the nearest: 1keV (for bin<10keV) or 5 keV (for 10keV<bin<50keV) or 10 keV (otherwise)
double EnergyScale_analysis::getOptimalBinWidthInMeV(double histogramRMS, double histogramIntegral) const {
  const auto resolution = 0.005; // [MeV]
  auto binOpt = histogramRMS / log(1+histogramIntegral);
  auto bin=std::max( resolution, binOpt );
  /////////////////
  //  const auto resolution = 0.005; // [MeV]
  //  auto binMin = std::max( resolution, histogramRMS/5); // at least 20 bins  in [-sigma, sigma] range
  //  auto binOpt = 5 * histogramRMS / log(1+histogramIntegral); // 4 * histogramRMS / log(1+histogramIntegral);
  //  auto bin=std::min( binMin, binOpt );
  /////////////////
  ///  auto binMin = std::max( resolution, histogramRMS/5); // at least 10 bins  in [-sigma, sigma] range
  ///  auto binOpt = 4 * histogramRMS / log(1+histogramIntegral); // 4 * histogramRMS / log(1+histogramIntegral);
  ///  auto bin=std::max( binMin, binOpt );
  ////////////////
  std::cout << __FUNCTION__ << ":  rms=" << histogramRMS << "  integral=" << histogramIntegral << "  |  binOpt=" << binOpt << "  binFinal=" << bin << std::endl;
  if (bin<0.01) return resolution * (long)( bin / resolution); // rounded off to 1 keV
  if (bin<0.02) return 2 * resolution * (long)( bin / resolution / 2); // rounded off to 2 keV
  if (bin<0.05) return 5 * resolution * (long)( bin / resolution / 5); // rounded off to 5 keV
  return 10 * resolution * (long)( bin / resolution / 10); // rounded off to 10 keV
...
std::tuple<double, size_t> EnergyScale_analysis::getSelectionChi2(EventCollection &selection, bool debug_histos_flag) {
  ...
  const double fitPeakFraction = 0.6; // 0.8; // In the 2nd pass limit fit range to the vicinity of the peak at a certain level
  // const double energyBinSize = getOptimalBinWidthInMeV(sqrt(pow(selection.expectedExcitationEnergySigmaInMeV, 2)+
  // 							    pow(myOptions.detectorExcitationEnergyResolutionInMeV, 2)),
  // 						       selection.events.size()); // [MeV]
  double energyBinSize = 0.002; // MeV
  const double xmin = 0.0; // [MeV]
  const double xmax = selection.expectedExcitationEnergyPeakInMeV + 5.0; // [MeV]
  const long nbins = (long)((xmax-xmin)/energyBinSize);
  TH1D hExcitationEnergy("hExcitationEnergy", "", nbins, xmin, xmin+nbins*energyBinSize);
  TH1D hExcitationEnergyAve("hExcitationEnergyAve", "", nbins, xmin, xmin+nbins*energyBinSize);
  ...
  for(auto &event: selection.events) {
    ...
    hExcitationEnergy.Fill( std::get<0>(result) ); // [MeV] - ExcitationEnergy in CMS
    hExcitationEnergyAve.Fill( std::get<1>(result) ); // [MeV] - average/expected ExcitationEnergy in CMS (=const)
    npassed++;
  }
  
  // rebin to get reasonable peak height
  double maxEnergyBinSize = 0.05;
  TH1D hExcitationEnergy_copy=hExcitationEnergy;
  int rbin=2;
  while(hExcitationEnergy.GetMaximum()<25 && energyBinSize<maxEnergyBinSize) {
    hExcitationEnergy = * dynamic_cast<TH1D*>(hExcitationEnergy_copy.Rebin(rbin, hExcitationEnergy_copy.GetName())); // do not modify
    energyBinSize=hExcitationEnergy.GetBinWidth(1);
    rbin++;
  };
  ...
  // post-fill fitting using bifurcated gaussian shape
  const bool use_fixed_peak = USE_SHAPE_CHI2;
  const bool hasEnoughEvents = true; // (hExcitationEnergy.Integral() > 1000); // arbitrary cut
  ...
  if(result1.Get()) {
	...
	pow(energyBinSize*energyBinSize, 2) +
	pow(myOptions.detectorExcitationEnergyResolutionInMeV, 2) +
  ...
  if(use_fixed_peak) {
    ...
    funcPtr->SetParLimits(1, 0.5*funcPtr->GetParameter(1), 1.5*funcPtr->GetParameter(1));
  }
