...
int main(int argc, char *argv[]) {
  ...
  TH2D *hUZProjection =  myUVWProjector.GetStripVsTime_TH2D(projection_type::DIR_U);
  TH2D *hVZProjection =  myUVWProjector.GetStripVsTime_TH2D(projection_type::DIR_V);
  TH2D *hWZProjection =  myUVWProjector.GetStripVsTime_TH2D(projection_type::DIR_W);
  ...
  for(int iBinX=1;iBinX<hUZProjection->GetNbinsX();++iBinX){
    for(int iBinY=1;iBinY<hUZProjection->GetNbinsY();++iBinY){
      ...
      evt.AddValByStrip(projection_type::DIR_U, strip_number, time_cell, value);

      value = hVZProjection->GetBinContent(iBinX, iBinY);
      evt.AddValByStrip(projection_type::DIR_V, strip_number, time_cell, value);

      value = hWZProjection->GetBinContent(iBinX, iBinY);
      evt.AddValByStrip(projection_type::DIR_W, strip_number, time_cell, value);
    }
  }

  evt.GetStripVsTime(projection_type::DIR_U)->Draw("colz");
  ...
  persistent_event->GetStripVsTime(projection_type::DIR_U)->Draw("colz");
