...
int main(int argc, char *argv[]) {
  ...
  for(unsigned int iEvent=0;iEvent<nEvents;++iEvent){
    ...
    int aDir = definitions::projection_type::DIR_V;
    ...
    for(int aDir=definitions::projection_type::DIR_U;aDir<=definitions::projection_type::DIR_W;++aDir){
      canvas->cd(2);
      gPad->cd(1+aDir);
    ...
    canvas->cd(3);
    gPad->cd(1);
    myUVWProjector.GetStripVsTime_TH2D(definitions::projection_type::DIR_U)->Draw("colz");
    canvas->cd(3);
    gPad->cd(2);
    myUVWProjector.GetStripVsTime_TH2D(definitions::projection_type::DIR_V)->Draw("colz");
    canvas->cd(3);
    gPad->cd(3);
    myUVWProjector.GetStripVsTime_TH2D(definitions::projection_type::DIR_W)->Draw("colz");
    */
    app.Run(kTRUE);
  }
