...
public:
	...
	UVWprojector(int n = 100, int nx = 25, int ny = 25);
	...
	std::shared_ptr<TH1D> GetStripProfile_TH1D(direction dir); // Get TH1D of time-integrated strip projection (SELECTED DIRECTION)
	std::shared_ptr<TH2Poly> GetStripProfile_TH2Poly();     // Get TH2Poly of time-integrated strip projection (ALL STRIPS)
	std::shared_ptr<TH2D> GetStripVsTime_TH2D(direction dir);  // Get TH2D of strip vs time projection (SELECTED DIRECTION)  
...
protected:
	...
	std::map<std::tuple<int, int> /* TH2 bin index [1..NX*NY] */, BinFracMap> fAreaFractionMap;
