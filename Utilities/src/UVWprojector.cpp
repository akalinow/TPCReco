...
bool UVWprojector::InitAreaMapping() {
  ...
  if( area_npoints<1 || !h2 || !geo_ptr || !(geo_ptr->IsOK()) || !(tp=geo_ptr->GetTH2Poly())) {
    ...
    if(_debug) {
      std::cerr << "InitAreaMapping: ERROR: Failed sanity checks (1): " 
		<< "NPOINTS=" << area_npoints << ", HIST_PTR=" << h2 << ", TH2POLY=" << tp << ", GEO_PTR=" << geo_ptr << std::endl;
    }
    // DEBUG
  ...
  for(int ibinx=0; ibinx<nxbins; ibinx++) {
	...
	BinFracMap &bmap = fAreaFractionMap[MultiKey2(ibinx,ibiny)];
...
bool UVWprojector::InitTimeMapping() {
  ...
  // sanity checks
  if( !h3 || !geo_ptr || !(geo_ptr->IsOK()) ) return false;

  const int time_nbins = geo_ptr->GetAgetNtimecells(); // number of time buckets in digitizing electronics
  ...
  const double delay = geo_ptr->GetTriggerDelay(); // electronics trigger delay [us]
  ...
  // DEBUG
  if(_debug) {
    std::map<int, BinFracMap>::iterator it;
    std::map<int, double>::iterator it2;
    for(it=fTimeFractionMap.begin(); it!=fTimeFractionMap.end(); it++) {
      for(it2=(it->second).FracMap.begin(); it2!=(it->second).FracMap.end(); it2++) {
...
TH2Poly* UVWprojector::GetStripProfile_TH2Poly() {
  ...
  TH2Poly *th2p = new TH2Poly(Form("%s_copy", tp->GetName()), tp->GetTitle(), 
