...
public:
  EventFilter<std::function<bool(EventMock &)>> filter;
  EventMock event;
  pt::ptree ptree;
};
  ...
  std::stringstream config{R"(
{
  ...
  filter.setConditions(ptree);
  filter.setEnabled(true);
  EXPECT_CALL(event.eventInfo, GetEventId()).Times(1).WillRepeatedly(Return(0));
  EXPECT_FALSE(filter.pass(event));
  ...
  EXPECT_TRUE(filter.pass(event));
  EXPECT_CALL(event.eventInfo, GetEventId()).Times(1).WillRepeatedly(Return(3));
  EXPECT_FALSE(filter.pass(event));
}