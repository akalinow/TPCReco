...
class SigClusterTPC {
 ...
 public:
  ...
  std::vector<MultiKey2> GetHitListByDir(int strip_dir) const; // list of SELECTED hits corresponding to a given STRIP_DIR[0-2], value=key(REBIN_TIME_CELL [0-511], STRIP_NUM [1-1024])
  ...
  bool CheckByStrip(int strip_dir, int strip_number, int time_cell) const;  // valid range [0-2][1-1024][0-511]
  ...
  // statistics
  long GetNhitsByStrip(int strip_dir, int strip_num) const;  // # of hits in a given strip
  long GetNhits(int strip_dir) const;                        // # of hits in a given direction
  long GetNhits() const;                                     // global # of hits
  int GetMultiplicity(int strip_dir) const;                  // # of strips with hits in a given direction
  int GetMultiplicity() const;                               // global # of strips with hits from all strips
  int GetMinStrip(int strip_dir) const; // minimal strip number in a given direction
  int GetMaxStrip(int strip_dir) const; // maximal strip number in a given direction
  int GetMinTime(int strip_dir) const;  // minimal RAW time cell in a given direction
  int GetMaxTime(int strip_dir) const;  // maximal RAW time cell in a given direction
  ...
  double GetMaxCharge(int strip_dir) const;      // maximal charge from all cluster hits in a given direction
  double GetMaxCharge(int strip_dir, int strip_number) const; // maximal charge from cluster hits in a given strip (if any)
  ...
  int GetMaxChargeTime(int strip_dir) const;     // arrival time of the maximal charge in a given direction
  ...
  double GetTotalCharge(int strip_dir, int strip_number) const; // charge integral from cluster hits in a given strip (if any)
  double GetTotalChargeByTimeCell(int strip_dir, int time_cell) const; // charge integral from a single time cell from all cluster hits in a given direction
