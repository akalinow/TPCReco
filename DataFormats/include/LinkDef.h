#ifdef __CINT__
#pragma link off all globals;
#pragma link off all classes;
#pragma link off all functions;
#pragma link C++ nestedclasses;

#pragma link C++ class EventTPC+;
#pragma link C++ class GeometryTPC+;
#pragma link C++ class StripTPC+;
#pragma link C++ class SigClusterTPC+;
#pragma link C++ class eventraw::EventRaw+;
#pragma link C++ class eventraw::EventInfo+;
#pragma link C++ class eventraw::EventData+;
#pragma link C++ class eventraw::AgetRaw+;
#pragma link C++ class eventraw::ChannelRaw+;

#pragma link C++ class Hit2D+;
#pragma link C++ class TrackSegment2D+;
#pragma link C++ class TrackSegment3D+;
#pragma link C++ class Track3D+;
#pragma link C++ class MultiKey2+;
#pragma link C++ class MultiKey3+;
#pragma link C++ class MultiKey3_uint8+;
#pragma link C++ class MultiKey4+;
#pragma link C++ class MultiKey5+;

#endif
